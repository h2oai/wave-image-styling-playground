...
def generate_frames(args: dict, source_latent, g_ema_list, output_prefix):

    alphas = np.linspace(0, 1, num=20)
    
    interpolate_func = interpolate_with_boundaries # default
    if 'target_latents' in args:                        # if provided with targets
        interpolate_func = interpolate_with_target_latents
    if 'unedited_frames' in args and args['unedited_frames'] > 0:                       # if only interpolating through generators
    ...
    for idx, latent in tqdm(enumerate(latents), total=len(latents)):
        ...
        img_path = f"{output_prefix}{str(idx).zfill(3)}.jpg"
        utils.save_image(img, img_path, nrow=1, normalize=True, scale_each=True, range=(-1, 1))
...
def duplicate_latent(args: dict, source_latent, alphas):
    return [source_latent for _ in range(args['unedited_frames'])]

def interpolate_with_boundaries(args: dict, source_latent, alphas):
...
def interpolate_with_target_latents(args: dict, source_latent, alphas):    
    # interpolate latent codes with all targets
...
if __name__ == '__main__':
    ...
    source_latent = np.load('./z_output/baby_1.npz')['w']
    ...
    for idx, ckpt_path in enumerate(args.ckpt):
        ...
        generate_frames(vars(args), source_latent, [g_ema], output_dir)
