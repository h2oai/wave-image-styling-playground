...
class DDIMSampler(object):
    ...
    def ddim_sampling(
        ...
        print(f"self.model.betas.device: {device}")
        b = shape[0]
        if x_T is None:
    ...
    def decode(
        ...
        x_dec = x_latent.to(self.model.device)
        for i, step in enumerate(iterator):
            index = total_steps - i - 1
            ts = torch.full((x_latent.shape[0],), step, device=self.model.device, dtype=torch.long)
